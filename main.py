...
class MyWindow(QMainWindow, Ui_client):
    def __init__(self):
        ...
        self.drawpoint = [585+960, 135+440]
        self.initial = True
    ...
    def paintEvent(self, e):
        try:
            ...
            qp.setPen(pen)
            qp.drawRect(485+960, 35+440, 200, 200)
            pen = QPen(QColor(0, 138, 255), 2, Qt.SolidLine)
            qp.setPen(pen)
            qp.drawLine(self.drawpoint[0], 35+440, self.drawpoint[0], 235+440)
            qp.drawLine(485+960, self.drawpoint[1], 685+960, self.drawpoint[1])
            self.label_point.move(self.drawpoint[0] + 10, self.drawpoint[1] + 10)
            pitch = round((self.drawpoint[1] - 135 - 440) / 100.0 * 20)
            yaw = round((self.drawpoint[0] - 585 - 960) / 100.0 * 20)
    ...
    def mouseMoveEvent(self, event):
        ...
        y = event.pos().y()
        # print(x,y)
        if x > 485+960 and x < 685+960 and y > 35+440 and y < 235+440:
            try:
                self.drawpoint[0] = x
                self.drawpoint[1] = y
                # self.update()
    ...
    def attitude(self, target1, target2):
        try:
            ...
            self.drawpoint[0] = 960 + 585 + self.slider_yaw.value() * 5
            self.drawpoint[1] = 440 + 135 + self.slider_pitch.value() * 5
            self.update()
            # self.repaint()
            # print(command)
        except Exception as e:
...
class ledWindow(QMainWindow, Ui_led):
    ...
    def mode1Color(self):
        if (self.radioButtonOne.isChecked() == True) or (self.radioButtonThree.isChecked() == True):
            command = cmd.CMD_LED + '#' + '255' + '#' + str(self.rgb[0]) + '#' + str(self.rgb[1]) + '#' + str(
                self.rgb[2]) + '\n'
            self.client.send_data(command)
            # print(command)
